...
def load_cache(embed_model: str) -> Tuple[np.ndarray, np.ndarray, List[Dict], Dict[int, str]]:
...
def cmd_ingest(args):
    ...
    json.dump({"doc_count": len(chunks), "comments": len(comments),
               "provider": provider.name, "embed_model": provider.embed_model, "kind": "sentence-chunks"},
...
def cmd_search(args):
    provider = get_provider(args)
    matrix, mean, chunk_docs, comments = load_cache(provider.embed_model)
    print(f"Loaded {len(chunk_docs)} chunks from {len(comments)} comments ({provider.name}).")
