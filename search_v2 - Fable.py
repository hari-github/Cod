...
SEMANTIC_MIN_COSINE   = 0.55  # cosine gate to become a candidate
                              # raise toward 0.70 if semantic pulls too many irrelevant docs
                              # lower toward 0.45 if you're missing relevant ones
SEMANTIC_AUTO_QUALIFY = 0.70  # cosine alone marks RELEVANT — skips LLM
...
FUZZY_BYPASS_MIN_COSINE = 0.45  # a phrase match must ALSO clear this cosine to bypass
                                # the LLM — filters passing mentions in off-topic comments
...
def semantic_search(
    ...
    Expects a row-normalized matrix (see l2_normalize); normalizes defensively
    if a raw one is passed.
    """
    if doc_matrix.size and not np.isclose(float(np.linalg.norm(doc_matrix[0])), 1.0, atol=1e-3):
...
def search(
    ...
    semantic_res, best_cos = semantic_search(variants, provider, doc_matrix, documents, min_cosine)
...
def main():
    ...
    doc_matrix = l2_normalize(build_or_load_embeddings(
