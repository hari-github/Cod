...
SEMANTIC_AGG_TOPK     = 2     # decision cosine = MEAN of the top-K variant cosines per doc.
                              # 1 = plain max (old behaviour, single variant can carry a doc);
                              # 2 = a 2nd variant must agree → less single-variant drift, but
                              # scores run lower, so re-check the thresholds above after changing.
...
# ── LEXICAL DECISION LAYER — pure fuzzy/phrase, applied BEFORE any cosine ─────
# This layer marks a candidate RELEVANT on lexical evidence ALONE (no cosine).
# It runs first in the rerank cascade; whatever it doesn't decide falls through
# to the cosine / LLM tiers below. Set a threshold to 101 to disable that rule.
# TRADE-OFF: dropping the cosine check means a passing mention ("no problem with
# prior auth, but billing was awful") can be marked RELEVANT. Keep thresholds
# strict (exact match) to limit that, or loosen them to send fewer to the LLM.
LEXICAL_PHRASE_RELEVANT = 100   # phrase_score ≥ this → RELEVANT (100 = exact contiguous
                                #   phrase; lower to 90 to also trust typo'd phrases)
LEXICAL_FUZZY_RELEVANT  = 100   # token_set_ratio (0-100) ≥ this → RELEVANT (100 = every
                                #   word of the term is present; lowering is riskier)
...
def semantic_search(
    ...
    sims_list: List[np.ndarray] = []     # one cosine row per variant
    ...
    for vec in qvecs:
        ...
        # RRF still uses per-variant ranks → recall stays wide (any variant counts)
    ...
    # Decision score = MEAN of the top-K variant cosines per doc (drift-resistant
    # vs plain max: a lone drifting variant can't carry a doc, a 2nd must agree).
    # K=1 reproduces the old max exactly.
    sims_stack   = np.vstack(sims_list)                          # (n_variants, n_docs)
    k            = min(SEMANTIC_AGG_TOPK, sims_stack.shape[0])
    topk         = np.partition(sims_stack, -k, axis=0)[-k:, :]  # K largest per doc
    best_cosines = topk.mean(axis=0).astype(np.float32)
...
def lexical_verdict(c: Dict) -> Optional[str]:
    """
    Pure lexical decision — fuzzy/phrase ONLY, no cosine. Runs first in the
    cascade. Returns the rule that fired ('phrase' / 'all-words') or None when
    lexical evidence is insufficient (candidate falls through to cosine/LLM).
    """
    if c.get("phrase_score", 0.0) >= LEXICAL_PHRASE_RELEVANT:
        return "phrase"
    if c.get("fuzzy_score", 0.0) * 100.0 >= LEXICAL_FUZZY_RELEVANT:
        return "all-words"
    return None
...
def llm_rerank(candidates: List[Dict], query: str, provider: Provider) -> List[Dict]:
    ...
    Cascade (first match wins):
      0. LEXICAL layer — lexical_verdict() decides RELEVANT on fuzzy/phrase alone,
         BEFORE any cosine is considered.
      1. semantic_score ≥ SEMANTIC_AUTO_QUALIFY                  → RELEVANT, no LLM
      2. phrase_score ≥ FUZZY_PHRASE_QUALIFY AND
         semantic_score ≥ FUZZY_BYPASS_MIN_COSINE               → RELEVANT, no LLM
         (catches typo'd phrases (90-99) that the strict lexical rule skips,
          using cosine as corroboration)
    ...
    needs_llm: List[Dict] = []
    for c in candidates:
        lex = lexical_verdict(c)                       # ── tier 0: pure lexical ──
        if lex:
            c["qualified_by"] = f"lexical-{lex}"
            auto.append(c)
            continue
    ...
    if auto:
        n_lex = sum(1 for c in auto if c["qualified_by"].startswith("lexical-"))
        n_cos = sum(1 for c in auto if c["qualified_by"] == "high-cosine")
        n_phr = sum(1 for c in auto if c["qualified_by"] == "phrase-match")
        print(f"        {len(auto)} auto-qualified — skipping LLM "
              f"(lexical {n_lex} · high-cosine {n_cos} · phrase-match {n_phr})")
